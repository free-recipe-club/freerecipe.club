...
| **Contributor profiles (git-derived)** — show recipes by contributor, auto-generated from git history | Community recognition without accounts. "See all 12 recipes by Michelle Weinfeld-Geller." | Low | Derived from `byline` field in recipe data. No authentication needed — just an index page grouped by author. |
